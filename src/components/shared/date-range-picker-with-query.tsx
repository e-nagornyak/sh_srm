...
export function DateRangePickerWithQuery({
  dateRange,
  dayCount,
  ...
  const handleSelect = (range: DateRange | undefined) => {
    // Check if both dates are the same using dayjs
    if (
      range &&
      range.from &&
      range.to &&
      ...
      // If the dates are the same, do not update the state
      return
    }
    setDate(range)
  }
  ...
  React.useEffect(() => {
    ...
    if (date?.from) {
      newSearchParams.set("start_date", format(date.from, "yyyy-MM-dd"))
    } else {
      newSearchParams.delete("start_date")
    }

    if (date?.to) {
      newSearchParams.set("end_date", format(date.to, "yyyy-MM-dd"))
    } else {
